...
app.get('/serve-manifest', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await respondToServeManifestRequest(req.query.name as string, req.query.platform as string);
    res.status(200).send('OK');
  } catch (e) {
...
async function respondToServeManifestRequest(name: string, platform: string = 'android') {
  switch (name) {
    case 'test-update-basic':
      await serveTestUpdate1(platform);
      break;
    case 'test-update-invalid-hash':
      await serveTestUpdateInvalidHash(platform);
      break;
    case 'test-update-with-invalid-asset-hash':
      await serveTestUpdateWithInvalidAssetHash(platform);
      break;
    case 'test-update-with-multiple-assets':
      await serveTestUpdateWithMultipleAssets(platform);
      break;
    case 'test-update-with-older-commit-time':
      await serveTestUpdateWithOlderCommitTime(platform);
      break;
    case 'test-update-before-rollback':
      await serveTestUpdateBeforeRollback(platform);
      break;
    case 'test-rollback':
      await serveRollback(platform);
      break;
    case 'test-update-for-asset-deletion':
      await serveTestUpdateForAssetDeletion(platform);
      break;
    default:
...
async function serveTestUpdate1(platform: string) {
...
async function serveTestUpdateInvalidHash(platform: string) {
...
async function serveTestUpdateWithInvalidAssetHash(platform: string) {
...
async function serveTestUpdateWithMultipleAssets(platform: string) {
  const bundleFilename = 'bundle2.js';
  const newNotifyString = 'test-update-2';
  const hash = await Update.copyBundleToStaticFolder(
    projectRoot,
...
async function serveTestUpdateWithOlderCommitTime(platform: string) {
...
async function serveTestUpdateBeforeRollback(platform: string) {
  const bundleFilename = 'bundle1.js';
...
async function serveRollback(platform: string) {
...
async function serveTestUpdateForAssetDeletion(platform: string) {
